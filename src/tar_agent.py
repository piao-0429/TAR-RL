...
class TARAgent:
    def __init__(self, obs_shape, action_shape, act_rep_dim, seq_len, device, use_sensor, lr, feature_dim, hidden_dim,
        ...
        with torch.no_grad():
            dummy_input = torch.zeros(1, 3, 84, 84).to(device)
        ...
        self.action_vae = ActionVAE(action_shape[0], act_rep_dim, hidden_dim).to(device)
        ...
        self.act_vae_opt = torch.optim.Adam(self.action_vae.parameters(), lr=lr)
        self.act_vae_ft_opt = torch.optim.Adam(self.action_vae.parameters(), lr=lr*0.1)
    ...
    def act(self, obs, step, eval_mode, obs_sensor=None, is_tensor_input=False, force_action_std=None):
        ...
        action, _, _ = self.action_vae.decode(act_rep)
    ...
    def update_representation(self, replay_iter, traj_iter, step, use_sensor):
        ...
        act_rep, _, _ = self.action_vae.encode(action)
        act_rec, _, _, _ = self.action_vae.forward(action)
        
        loss_rec = F.mse_loss(act_rec, action)
        
        act_rep_forward = self.inv_dyn(torch.cat([ob_rep, ob_rep_next], dim=1))
        
        loss_aln = F.mse_loss(act_rep_forward, act_rep)
        
        with torch.no_grad():
            batch_seq_rep, _, _ = self.action_vae.encode(batch_seq.reshape(-1, batch_seq.shape[-1]))
            batch_seq_rep = batch_seq_rep.reshape(batch_seq.shape[0],self.seq_len, self.act_rep_dim)
        logits = self.task_cls(batch_seq_rep)
        loss_cls = F.binary_cross_entropy_with_logits(logits, batch_label)
        
        loss = loss_rec + self.cls_weight * loss_cls + self.aln_weight * loss_aln
        
        metrics['loss_rec'] = loss_rec.item()
        metrics['loss_cls'] = loss_cls.item()
        metrics['loss_aln'] = loss_aln.item()
        
        self.act_vae_opt.zero_grad()
        self.task_cls_opt.zero_grad()
        self.inv_dyn_opt.zero_grad()
        loss.backward()
        self.act_vae_opt.step()
